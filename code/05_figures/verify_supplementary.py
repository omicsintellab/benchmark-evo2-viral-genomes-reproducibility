#!/usr/bin/env python3
"""verify_supplementary.py — le o DOCX suplementar (SOMENTE LEITURA) e confere cada celula
contra o artefato que a produziu: os JSONs de results/json/ e as tabelas de results/tables/
deste repositorio. Nao reescreve nada. O .docx nao e versionado aqui: use o Supplementary
Material publicado pela revista, ou o gerado por make_supplementary.py.

A guarda de COBERTURA no fim existe porque a primeira versao deste script comparava a tabela
ERRADA por deslocamento de indice e passava em silencio: zero divergencias porque zero
comparacoes. Cada tabela declara um minimo de checagens.

Uso:  python code/05_figures/verify_supplementary.py --docx <Supplementary_Material.docx>
"""
...
ap.add_argument("--docx", required=True,
                help="Supplementary Material .docx (o artefato submetido; nao e versionado aqui)")
ap.add_argument("--root", default=ROOT, help="raiz deste repositorio")
args = ap.parse_args()
R = P = args.root
J = lambda f: json.load(open(f"{R}/results/json/{f}"))

x = zipfile.ZipFile(args.docx).read("word/document.xml").decode()
# tabelas -> lista de linhas -> lista de celulas (texto)
tables = []
for tbl in re.findall(r"<w:tbl>.*?</w:tbl>", x, re.S):
    ...
    for tr in re.findall(r"<w:tr[ >].*?</w:tr>", tbl, re.S):
        ...
        rows.append(cells)
    tables.append(rows)
print(f"tabelas lidas do docx: {len(tables)}")
...
# ---------- S1: composicao do corpus (soma das linhas/colunas)
t1 = tables[0]
...
for r in body:
    ...
    eq(f"S1 soma da linha {r[0]}", sum(vals[:4]), vals[4])
...
if tot_row:
    ...
    for i,lbl in enumerate(["Eukaryote","Bacteria","Archaea","Unknown","Total"]):
        eq(f"S1 total da coluna {lbl}", col_sums[i], tv[i])
eq("S1 total geral = 19,429", col_sums[4], 19429)

# ---------- S2: selecao de amostras (aritmetica declarada)
t2 = tables[1]
g = {r[0].strip(): num(r[1]) for r in t2[1:] if len(r)>1 and num(r[1]) is not None}
eq("S2 Baltimore 981", g.get("Baltimore probe subset"), 981)
eq("S2 uniao 1,912 = 981+1200-269", g.get("Union of both subsets"), 981+1200-269)
...
for r in tables[10][1:]:
    ...
    eq(f"S10 {tgt} n familias", num(r[4]), len(fams))
    eq(f"S10 {tgt} sd ratio", num(r[5]),
       st.median([f["sd_within"]/wf[tgt]["sd_global"] for f in fams.values()]), tol=0.006)

print(f"\nchecagens numericas: {checks} | divergencias: {len(fails)}")
for f in fails: print("  X", f)

# ================== SEGUNDA RODADA: S3, S4, S7, S9 ==================
print("\n--- S3/S4/S9 contra supplementary_tables.md do repo publico ---")
...
for docx_idx, header, label in ((2, "Table S1.", "S3"), (3, "Table S2.", "S4"), (9, "Table S3.", "S9")):
    ...
    eq(f"{label} n de linhas", len(got_body), len(src_body))
    for gr, sr in zip(got_body, src_body):
        for gc, sc in zip(gr[1:], sr[1:]):
            a, b = num(gc), num(sc)
            if a is not None and b is not None:
                eq(f"{label} linha '{sr[0][:22]}'", a, b)

print("\n--- S7 contra ci_consistency.vs_best_baseline_by_class ---")
